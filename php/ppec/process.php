...
require '../sql.php';
    ...
    foreach($_POST['item_name'] as $key=>$itmname)
    {
		...
		$query="select product,id,price from cart where id='$product_code' and username='".$_SESSION['username']."'";
        ...
        $paypal_data .= '&L_PAYMENTREQUEST_0_NAME'.$key.'='.urlencode($obj['product']);
		...
		$paypal_product['items'][] = array('itm_name'=>$obj['product'],
											'itm_price'=>$obj['price'],
	...
	$padata = 	'&METHOD=SetExpressCheckout'.
				...
				'&LOGOIMG=http://localhost/upload%20mini%20prj/css/pplogo.png'. //site logo
				...
				*/
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
				{
					...
					*/
					...
					//print_r($httpParsedResponseAr);
					echo '</pre>';
					echo '<form action="http://localhost/online_store_php-11-complete/php">';
